...
struct GravitasSketch : p5::Sketch
{
    ...
    // ── Effects ───────────────────────────────────────────────────────────────
    struct Particle { float x, y, vx, vy, life, maxLife, size; color_t col; };
    struct Ring     {
        float x, y, startR, endR, life, maxLife;
        color_t col;
        float curR() const { return startR + (endR - startR) * (1.f - life / maxLife); }
    };
    struct FloatText { float x, y, vy, life, maxLife; std::string text; color_t col; };
    ...
    // ── Background stars ──────────────────────────────────────────────────────
    struct BgStar { float x, y, brightness, phase, size; };
    ...
    color_t hsv(float h, float s, float v, float a = 1.f)
    {
        ...
        return color((int)((r + m) * 255), (int)((g + m) * 255),
                     (int)((b + m) * 255), (int)(a * 255));
    }
    ...
    {
        state     = State::Playing;
        score     = 0;  combo     = 0;  comboTimer = 0.f;  lives = 5;
        seedTimer = 0.f;  seedIv  = 2.8f;
        voidTimer = 16.f;
        pulseCd   = 0.f;  pulseR  = -1.f;
        starMass  = 1.0f;
        starX     = W * 0.5f;
        starY     = H * 0.5f;
        shakeDur  = 0.f;
        seeds.clear();  voidRifts.clear();
        particles.clear();  rings.clear();  floatTexts.clear();
    }
    ...
    void firePulse()
    {
        ...
        pulseCd = PULSE_CD;
        pulseR  = 0.f;
        rings.push_back({starX, starY, 0.f, PULSE_MAX_R, 0.55f, 0.55f,
                         color(255, 215, 80)});
    }
    ...
    void draw() override
    {
        ...
        if (state == State::Menu)     { drawBgStars(); drawMenu();     return; }
        if (state == State::GameOver) { drawBgStars(); drawScene(false); drawGameOver(); return; }
    ...
    void updateSeeds(float dt)
    {
        for (auto& s : seeds) {
            ...
            if (spd > 330.f) { s.vx = s.vx / spd * 330.f; s.vy = s.vy / spd * 330.f; }
        ...
        std::erase_if(seeds, [](const Seed& s) { return !s.alive; });
    }
    ...
    void seedCollected(const Seed& s, Portal& p)
    {
        ...
        spawnParticles(s.x, s.y,  6, color(255, 255, 255, 200));
    ...
    void seedLost(const Seed& s)
    {
        ...
        spawnParticles(s.x, s.y,  5, color(200, 60, 60, 180));
        rings.push_back({s.x, s.y, 0.f, 50.f, 0.6f, 0.6f, color(200, 50, 50)});
        floatTexts.push_back({s.x, s.y - 10.f, -32.f, 1.2f, 1.2f, "LOST", color(255, 70, 50)});
    }
    ...
    void updateVoidRifts(float dt)
    {
        ...
        std::erase_if(voidRifts, [](const VoidRift& v) { return !v.alive; });
    }
    ...
    void spawnSeed()
    {
        ...
        float m    = 24.f;
        if      (edge < 1.f) { s.x = fr(m, W - m); s.y = -m; }
        else if (edge < 2.f) { s.x = fr(m, W - m); s.y = H + m; }
        else if (edge < 3.f) { s.x = -m;            s.y = fr(m, H - m); }
        else                 { s.x = W + m;          s.y = fr(m, H - m); }
    ...
    void spawnVoidRift()
    {
        ...
        rings.push_back({vr.x, vr.y, 0.f, 90.f, 0.8f, 0.8f, color(110, 0, 160)});
    }
    ...
    void spawnParticles(float x, float y, int n, color_t col)
    {
        for (int i = 0; i < n; ++i) {
            ...
            particles.push_back({x, y, std::cos(a) * s, std::sin(a) * s,
                                  l, l, fr(1.5f, 4.5f), col});
        }
    }
    ...
    void updateRings(float dt)
    {
        ...
        std::erase_if(rings, [](const Ring& r) { return r.life <= 0.f; });
    }

    void updateFloatTexts(float dt)
    {
        for (auto& f : floatTexts) { f.y += f.vy * dt; f.life -= dt; }
        std::erase_if(floatTexts, [](const FloatText& f) { return f.life <= 0.f; });
    }
    ...
    void drawSeeds()
    {
        ...
        for (auto& s : seeds) {
            ...
            float urgency = (lifeT < 0.28f) ?
                            (0.5f + 0.5f * std::sin(bgTime * 13.f)) : 1.f;
    ...
    void drawPlayerStar()
    {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            float hw     = 3.2f;
            beginShape();
            vertex(std::cos(ang) * baseR * 0.55f - std::sin(ang) * hw,
                   std::sin(ang) * baseR * 0.55f + std::cos(ang) * hw);
