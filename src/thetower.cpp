...
struct TowerSketch : p5::Sketch
{
    ...
    void buyUpgrade(int id)
    {
        ...
        spawnRing({CX, CY}, color(100, 255, 180), 30.f, 90.f, 0.6f);
        spawnFloatingText({CX, CY - 30.f}, std::string(up.name) + " upgraded!", color(100, 255, 180), 1.2f);
    }
    ...
    void update()
    {
        ...
        if (waveTimer >= 20.f) {
            ...
            waveBannerT = 2.2f;
            spawnRing({CX, CY}, color(255, 200, 60), 40.f, 160.f, 0.8f);
        }
    ...
    void updateEnemies(float dt)
    {
        for (auto& e : enemies) {
            ...
            if (length(e.pos - float2 {CX, CY}) < TOWER_RADIUS + e.size * 0.6f) {
                ...
                damageFlashT = 1.f;
                triggerShake(0.3f);
                spawnParticles(e.pos, color(255, 80, 60), 14);
                spawnRing(e.pos, color(255, 80, 60), 0.f, 50.f, 0.4f);
            }
        }
    ...
    void fireBullet(float2 dir, BulletType type)
    {
        ...
        tl.col = (type == BulletType::Multi) ? color(80, 220, 255) : (type == BulletType::Speed) ? color(255, 160, 40)
                                                                                                 : color(255, 230, 80);
    ...
    void updateShooting(float dt)
    {
        ...
        if (inRange.size() >= 2 && roll(rng) < multiShotChance()) {
            ...
            spawnFloatingText({CX, CY - 55.f}, "MULTI!", color(100, 200, 255), 0.7f);
        } else {
        ...
        if (!speedShotActive && roll(rng) < speedShotChance()) {
            ...
            speedIndicatorT = 1.f;
            chargeFlashT = 1.f;
            spawnRing({CX, CY}, color(80, 255, 200), 28.f, 80.f, 0.4f);
            spawnFloatingText({CX, CY - 68.f}, "SPEED SHOT!", color(80, 255, 200), 0.9f);
        }
    }
        ...
        for (auto& b : bullets) {
            ...
            for (auto& e : enemies) {
                ...
                if (length(b.pos - e.pos) < e.size * 0.75f + BULLET_R) {
                    ...
                    e.flashTimer = 1.f;
                    e.hp -= bulletDamage();
                    spawnParticles(b.pos, color(255, 200, 80), 5);
                    if (e.hp <= 0.f) {
                        e.alive = false;
                        int pts = 10 * wave;
                        score += pts;
                        ...
                        spawnRing(e.pos, color(255, 160, 40), 0.f, 65.f, 0.55f);
                        spawnRing(e.pos, color(255, 220, 100), 0.f, 40.f, 0.35f);
                        spawnFloatingText(e.pos, "+" + std::to_string(pts) + "  (" + std::to_string(drop) + "c)", color(255, 220, 80), 0.95f);
                    }
                    break;
                }
            }
    ...
    {
        spawnParticles(pos, color(255, 140, 40), 22);
        for (int i = 0; i < 8; ++i) spawnSquareShard(pos, color(255, 80, 40));
        for (int i = 0; i < 3; ++i) spawnSquareShard(pos, color(255, 200, 80));
        for (int i = 0; i < 12; ++i) {
            float a = TAU * i / 12.f;
            Particle p;
            p.pos = pos;
            p.vel = {std::cos(a) * 170.f, std::sin(a) * 170.f};
            p.col = color(255, 220, 100, 150);
            p.size = 3.f;
    ...
    void drawHPRing(float cx, float cy, float r)
    {
        ...
        color_t hpCol = lerp(color(255, 50, 50), color(60, 230, 120), ratio);
    ...
    void drawEnemies()
    {
        for (const auto& e : enemies) {
            ...
            color_t col = color(255, (int)(70.f * hpRatio), 40);
            ...
            if (e.flashTimer > 0.f) col = lerp(col, color(255, 255, 255), e.flashTimer);
    ...
    void drawBullets()
    {
        ...
        for (const auto& b : bullets) {
            ...
            color_t outerCol = (b.type == BulletType::Multi) ? color(60, 220, 255) : (b.type == BulletType::Speed) ? color(255, 160, 40)
                                                                                                                   : color(255, 230, 80);
            color_t coreCol = (b.type == BulletType::Multi) ? color(200, 245, 255) : (b.type == BulletType::Speed) ? color(255, 220, 150)
                                                                                                                   : color(255, 255, 210);
    ...
    void drawShop()
    {
        ...
        for (int id = 0; id < UP_COUNT; ++id) {
            ...
            noFill();
            blendMode(BlendMode::additive);
            color_t bc = mx ? color(80, 255, 80, sel ? 200 : 80) : buy ? color(60, 200, 255, sel ? 220 : 90)
                                                                       : color(80, 80, 120, sel ? 160 : 60);
            ...
            if (mx) {
                ...
                color_t cc = buy ? color(255, 210, 50) : color(160, 120, 60);
