...
Two different resource-management models coexist, by design:

- **`Texture`, `Framebuffer`, `Shader`** (`src/p5cpp/graphics/texture.cpp`, `framebuffer.cpp`, `shader.cpp`) are plain aggregates with no automatic cleanup, mirroring raylib's `Texture2D`/`RenderTexture2D`/`Shader` — `loadTexture()`/`loadImage()`/`createFramebuffer()`/`loadShader()` must be paired with an explicit `unload(Texture&)`/`unload(Framebuffer&)`/`unload(Shader&)` once the resource is no longer needed (see README.md's "Resource management" section). `loadShader()` does **not** deduplicate by source — every call compiles and links an independent GL program, even with identical source, unlike `loadFont()` below. A `RenderGroup`'s recorded ops (see `render_group_impl.hpp`) and `RenderState`'s active `shader` reference textures/shaders by id only, without keeping them alive — see the ownership note on `buildRenderGroup()` (`p5cpp.hpp`).
- **`Font`, `Sound`, `AudioStream`** stay `shared_ptr`-backed handles with automatic cleanup, no `unload()` needed. `loadFont()` (`src/p5cpp/graphics/font.cpp`) caches by path/source: calling it again with the same arguments while a previous handle from that call is still alive returns a new lightweight handle sharing the same underlying parsed font, instead of reloading from disk. `loadImage()`/`loadTexture()` are deliberately **not** cached this way, since `Texture::upload()` is mutable and aliasing would make one texture's pixel upload silently visible through another.
